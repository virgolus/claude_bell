...
- **AskUserQuestion answered in the terminal**: selecting an option emits a `tool_result`, **not** a `UserPromptSubmit`, so it doesn't trigger the usual notification dismissal. Claude Code's `idle_prompt` for that wait also arrives ~60s late, sometimes after the answer — which would strand an interactive card nothing clears. The Notification handler drops a would-be question card when `transcriptLastAskUserQuestionAnswered` shows the last `AskUserQuestion` already has a matching `tool_result`. Cards shown while the question was still pending are cleared by the next session hook (`PreToolUse`/`Stop` → `sessionAdvanced`).
- **Terminal frontmost at stop time → do NOT hold** (final, 2026-06-05). The decision went back and forth, so the reasoning is recorded in full:
  - *Original:* skip the hold when a terminal is frontmost, to avoid queueing a terminal-typed reply behind the held hook.
  - *Reverted to always-hold (2026-06-04):* not-holding meant panel `Send`/option replies fell back to `TerminalBridge`, whose tty/tab targeting was then unreliable → wrong tab.
  - *Restored to not-hold (2026-06-05):* always-hold made terminal-typed replies linger — empirically (`/tmp/claudebell-debug.log`) `UserPromptSubmit` only fires once the hold releases, and if the user stays in the terminal there is no app-switch to trigger focus-release, so the hold sat until its 6-min timeout (notification stuck, reply queued/unprocessed). Meanwhile the `TerminalBridge` fallback was made reliable (persisted per-session tty + owning-app routing + send-time validation), so not-holding no longer costs correct targeting. Net: terminal frontmost → don't hold (typed reply clears the card instantly via `UserPromptSubmit`; a panel reply lands in the right tab via the fixed fallback). ClaudeBell/other app frontmost → hold (panel reply uses the instant, exact hook channel).
