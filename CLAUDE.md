...
2. **Re-sign after updating the binary — with the stable `ClaudeBell Self-Signed` identity, NOT ad-hoc**: `swift build` only signs the binary ad-hoc (linker-signed). The surrounding `.app` has no `CodeResources`, so quarantined downloads get rejected as "damaged and can't be opened". Always run `codesign --force --deep --sign "ClaudeBell Self-Signed" ClaudeBell.app` after copying the binary, then verify with `codesign --verify --deep --strict ClaudeBell.app` (must exit 0). Use `ditto -c -k --keepParent` instead of `zip` to preserve the signature envelope. **Do not use `--sign -` (ad-hoc):** the panel-reply paste fallback needs an effective Accessibility grant, and macOS binds that grant to the code's designated requirement. Ad-hoc keys it to the cdhash, so every rebuild silently invalidates AX (the toggle still shows "on" but `AXIsProcessTrusted()` is false and the `Cmd+V` paste no-ops). The self-signed cert gives a stable requirement (`certificate leaf = H"…"`) so the grant survives rebuilds. The identity lives in the signing machine's login keychain; never regenerate it (a new leaf hash forces every user to re-grant Accessibility).
